...
```bash
pnpm build        # vite build + the .d.ts
...
```
src/
├── components/      UI components (form, cards, modals, shared, widgets)
├── composables/     useApiCall
├── constants/       DEFAULT_ITEMS_PER_PAGE
├── editor/          Rich text editor (tiptap), in its own entry point
...
├── injection-keys.ts
├── plugin.ts        Registers the global components
...
- The kit ships no `AppLogo`: each app provides its own through `LogoComponentKey`, and where
  it links through the optional `LogoLinkKey`
- Components registered globally by `plugin.ts` (`ParentCard`, `FormCard`, `BreadcrumbBar`,
  `PageHeader`, `TagChip`, `ConfirmationModal`, `PillTabs`, `TextItem`) are also exported, so an app
  can import them explicitly if it prefers
- `vue/multi-word-component-names` is on: a single-word name here becomes a single-word global
  component in every app that installs the kit
...
`AppInput`, `AppSelect`, `AppAutocomplete`, `AppTextarea`, `AppPasswordInput` and `AppColorPicker`
wrap the Vuetify inputs with the shared defaults (`variant="outlined"`,
...
```bash
pnpm build        # mandatory: dist is versioned
pnpm type-check
pnpm lint
pnpm format:check
```
